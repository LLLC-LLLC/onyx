...
export default function SecurityHardeningPage() {
  ...
  // The lockdown toggles only enforce on password (BASIC) deployments, so the
  // card hides where the backend would refuse the save.
  const showPasswordLockdown =
    !isMultiTenant && authTypeMetadata.authType === AuthType.BASIC;
    ...
    <SettingsLayouts.Root>
      ...
      <SettingsLayouts.Body>
        ...
        <div className="flex w-full flex-col gap-3">
          ...
          {/* Password lockdown (single-tenant BASIC only). Two-stage by design
