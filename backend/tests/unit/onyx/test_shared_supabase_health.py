    ...
    result = _run_profile_process(
        ...
        "\n".join(
            (
                "import asyncio",
                "from onyx.shared_supabase_health import app",
                "async def status_for(path, raw_path, query_string=b''):",
                "    messages = []",
                "    async def receive():",
                "        return {'type': 'http.request', 'body': b'', 'more_body': False}",
                "    async def send(message):",
                "        messages.append(message)",
                "    await app({'type': 'http', 'method': 'GET', 'path': path, 'raw_path': raw_path, 'query_string': query_string}, receive, send)",
                "    return messages[0]['status']",
                "assert asyncio.run(status_for('/health', b'/health')) == 200",
                "assert asyncio.run(status_for('/health/', b'/health/')) == 200",
                "assert asyncio.run(status_for('//health', b'//health')) == 503",
                "assert asyncio.run(status_for('///health', b'///health')) == 503",
                "assert asyncio.run(status_for('///health///', b'///health///')) == 503",
                "assert asyncio.run(status_for('/health/', b'/health%2F')) == 503",
                "assert asyncio.run(status_for('/health', b'/%68ealth')) == 503",
                "assert asyncio.run(status_for('/health', None)) == 503",
                "assert asyncio.run(status_for('/health', b'/health', b'probe=true')) == 503",
            )
        ),
    )
